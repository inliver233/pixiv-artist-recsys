...
class RelatedArtistCandidateService:
    ...
    def build_candidates(self, *, seed_user_id: int, max_related_per_artist: int = 5, max_related_per_illust: int = 5) -> CandidateArtistResult:
        ...
        for artist_id in sorted(followed_artist_ids):
            ...
            for user in related_users.items[:max_related_per_artist]:
                if user.user_id in followed_artist_ids:
                    continue
            ...
            for illust_id in self.repository.list_illust_ids_for_artist(artist_user_id=artist_id)[:max_related_per_artist]:
                ...
                for illust in related_illusts.items[:max_related_per_illust]:
                    if illust.user_id in followed_artist_ids:
                        continue
