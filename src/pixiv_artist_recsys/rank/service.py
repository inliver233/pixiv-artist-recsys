...
class HeuristicArtistRankService:
    ...
    def rank_from_store(self, *, seed_user_id: int, max_results: int = 20) -> RankedRecommendationResult:
        ...
        for candidate_user_id, evidences in evidence_map.items():
            ...
            if artist is None:
                continue
            tags = self.repository.fetch_artist_tags(artist_user_id=candidate_user_id)
            ...
            quality_score = self._quality_score(illusts)
            final_score = 0.45 * tag_score + 0.35 * evidence_score + 0.20 * quality_score
            confidence = min(1.0, 0.25 + 0.15 * len(evidences) + 0.1 * min(3, len(illusts)))
            top_illust_ids = [illust.illust_id for illust in illusts[:3]]
